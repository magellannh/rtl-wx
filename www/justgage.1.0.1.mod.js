...
JustGage = function(config) { 
  ...
  this.config = 
  {
    ...
    // valueSymbol: string
    // string tacked on to end of value for display
    valueSymbol : (config.valueSymbol) ? config.valueSymbol : "",
  ...
  // title  
  //var titleFontSize = ((widgetH / 8) > 10) ? (widgetH / 10) : 14;
  var titleFontSize = ((widgetH / 10) > 14) ? (widgetH / 10) : 14;
  ...
  // label 
  var labelFontSize = ((widgetH / 16) > 11) ? (widgetH / 16) : 11; // JR - Changed min font from 10 to 11
  var labelX = dx + widgetW / 2;
  //var labelY = dy + widgetH / 1.126760563380282;
  var labelY = valueY + valueFontSize / 2 + 10; // JR was  6 instead of 10
  ...
  // value
  if (this.config.valueSymbol == "$") // JR - If Value Symbol is $, the put it first
    this.txtValue = this.canvas.text(this.params.valueX, this.params.valueY,this.config.valueSymbol+this.originalValue );
  else
    this.txtValue = this.canvas.text(this.params.valueX, this.params.valueY, this.originalValue + this.config.valueSymbol);
  this.txtValue. attr({
  ...
  // min
  this.txtMin = this.canvas.text(this.params.minX, this.params.minY, "");//this.config.min);  JR Hide min 
  this.txtMin. attr({
  ...
  // max
  this.txtMax = this.canvas.text(this.params.maxX, this.params.maxY, "");//this.config.max); JR Hide max
  this.txtMax. attr({
...
// refresh gauge level
JustGage.prototype.refresh = function(val, label) {
  // overflow values
  originalVal = val;
  ...
  if (this.config.valueSymbol == '$') // JR if symbol is $, show it before value
      this.canvas.getById(this.config.id+"-txtvalue").attr({"text":this.config.valueSymbol+originalVal});
  else
      this.canvas.getById(this.config.id+"-txtvalue").attr({"text":originalVal+this.config.valueSymbol});
  this.canvas.getById(this.config.id+"-txtlabel").attr({"text":label});
